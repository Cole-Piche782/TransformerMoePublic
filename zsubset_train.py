...
shouldJustHelpPickClusterCenters = True
...
def estimate_loss():
    ...
    out = {}
    model.eval()
    print("model.wte 0 0: " + str(getModelTransformer(model).wte.weight[0][0]))
    print("model block 2 0 0: " + str(getModelTransformer(model).h[2].attn.c_attn.weight[0][0]))
    for split in ['train', 'val']:
        losses = torch.zeros(eval_iters)
        percentsCorrect = torch.zeros(eval_iters)
        startTime = 0
...
def estimate_expert_loss():
    ...
    for split in ['train', 'val']:
        losses = torch.zeros(eval_iters)
        percentsCorrect = torch.zeros(eval_iters)
        routerLosses = torch.zeros(eval_iters)
        losses0 = torch.zeros(eval_iters)
        losses1 = torch.zeros(eval_iters)
        routerPercentsCorrect = torch.zeros(eval_iters)
        percentsCorrect0 = torch.zeros(eval_iters)
        percentsCorrect1 = torch.zeros(eval_iters)
        ...
        for i in range(eval_iters):
            ...
            with ctx:
                ...
                #Z = weightClasses(targetsRouter, Z)
            ...
            losses[i] = loss
            percentsCorrect[i] = moePercentCorrect
            routerLosses[i] = lossRouter
            routerPercentsCorrect[i] = routerPercentCorrect
            losses0[i] = loss0
            losses1[i] = loss1
            percentsCorrect0[i] = expert0PercentCorrect
            percentsCorrect1[i] = expert1PercentCorrect
        out[split] = losses.mean()
        print("loss for current split is: " + str(out[split]))
        print("routerLoss for current split is: " + str(routerLosses.mean()))
        print("loss0 for current split is: " + str(losses0.mean()))
        print("loss1 for current split is: " + str(losses1.mean()))
        print("percentCorrect for current split is: " + str(percentsCorrect.mean()))
        print("routerPercentCorrect for current split is: " + str(routerPercentsCorrect.mean()))
        print("percentCorrect0 for current split is: " + str(percentsCorrect0.mean()))
        print("percentCorrect1 for current split is: " + str(percentsCorrect1.mean()))
...
def initModel():
    ...
    if init_from == 'scratch':
        ...
        for item in state_dict:
            firstVal = "N/A"
            if(str(item) == "transformer.wte.weight"):
                print("wte item 0 0: " + str(state_dict[item][0][0]))
                for i in range(0, len(state_dict[item])):
                    wteWeights.append(state_dict[item][i])
            if(str(item) == "transformer.wpe.weight"):
                print("wpe item 0 0: " + str(state_dict[item][0][0]))
            # Check if the item is a tensor and has values
            if hasattr(state_dict[item], 'data') and state_dict[item].data.numel() > 0:
                firstVal = str(state_dict[item].data.view(-1)[0].item())
            print("item: " + str(item) + " first val: " + firstVal)



        with torch.no_grad():
            model.load_state_dict(state_dict)
            for i in range(0, len(wteWeights)):
                getModelTransformer(model).wte.weight[i] = wteWeights[i]
        iter_num = checkpoint['iter_num']
        best_val_loss = checkpoint['best_val_loss']
        print("best_val_loss: " + str(best_val_loss))
        print("model.wte 0 0: " + str(getModelTransformer(model).wte.weight[0][0]))
        print("model.wpe 0 0: " + str(getModelTransformer(model).wpe.weight[0][0]))

        print("model block 2 0 0: " + str(getModelTransformer(model).h[2].attn.c_attn.weight[0][0]))
        
        #print("wte[0][0]: " + str(model.transformer.wte.weight[0][0]))
...
tempStart = time.time()
#print("wte[0][0]: " + str(getModelTransformer(model).wte.weight[0][0]))
#exit()
while expertNum < 2:
