...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None, mask=None):
        if(mask is None):
            print("error, no mask passed, always pass a mask to forward")
            exit()
        device = idx.device
        b, t = idx.size()
        ...
        if targets is not None:
            ...
            # Create a masked targets tensor where masked positions are set to ignore_index
            masked_targets = torch.where(Z_flat == 1, targets_flat, torch.tensor(ignore_index).to(targets_flat.device))
            contains_nan = torch.isnan(masked_targets).any()
            contains_inf = torch.isinf(masked_targets).any()
            if contains_nan or contains_inf:
                print("masked_targets contains nan or inf")
                print("logits_flat: ", logits_flat)
                print("masked_targets: ", masked_targets)
                print("contains_nan: ", contains_nan)
                print("contains_inf: ", contains_inf)
                exit()
            contains_nan = torch.isnan(logits_flat).any()
            contains_inf = torch.isinf(logits_flat).any()
            if contains_nan or contains_inf:
                print("logits_flat contains nan or inf")
                print("masked_targets: ", masked_targets)
                print("logits_flat: ", logits_flat)
                print("contains_nan: ", contains_nan)
                print("contains_inf: ", contains_inf)
            ...
            contains_nan = torch.isnan(logits).any()
            contains_inf = torch.isinf(logits).any()
            if contains_nan or contains_inf:
    ...
    def get_cross_entropy(self, logits, targets, ignore_index):
    ...
    @torch.no_grad()
    def generateNoNew(self, idx, temperature=1.0, top_k=None):
        """
        Take a conditioning sequence of indices idx (LongTensor of shape (b,t)) and complete
        the sequence max_new_tokens times, feeding the predictions back into the model each time.
        Most likely you'll want to make sure to be in model.eval() mode of operation for this.
